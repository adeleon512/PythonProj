...
def lookup_user(dbc, name):
    ...
    with dbc, dbc.cursor() as cur:
        cur.execute('''
            ...
            FROM bug_user WHERE user_name = %s
        ''', (name,))
        row = cur.fetchone()
        if row is None:
            return None
...
def create_user(dbc, username, password, display_name, e_mail, role):
    """
    Creates a user.
    ...
    with dbc, dbc.cursor() as cur:
        cur.execute('''
            INSERT INTO bug_user (user_name, pw_hash, display_name,
                                  e_mail, role)
            VALUES (%s, %s, %s, %s, %s)
            RETURNING user_id
        ''', (username, hash.decode('UTF-8'), display_name, e_mail, role))
        row = cur.fetchone()
        return row[0]
